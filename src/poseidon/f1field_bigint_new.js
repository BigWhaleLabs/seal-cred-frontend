...
import * as FFFT from 'poseidon/fft.js'
import { getRandomBytes } from 'poseidon/random.js'
import buildSqrt from 'poseidon/fsqrt.js'
...
export default class ZqField {
  constructor(p) {
    ...
    this.p = BigInt(p)
    this.m = 1
    this.negone = this.p.minus(BigInt(1))
    this.two = BigInt(2)
    this.half = this.p.shiftRight(1)
    this.bitLength = this.p.bitLength()
    this.mask = BigInt(1).shiftLeft(this.bitLength).minus(BigInt(1))

    this.n64 = Math.floor((this.bitLength - 1) / 64) + 1
    this.n32 = this.n64 * 2
    this.n8 = this.n64 * 8
    this.R = BigInt(1).shiftLeft(this.n64 * 64)
  ...
  land(a, b) {
    return a.isZero() || b.isZero() ? BigInt(0) : BigInt(1)
  }
  ...
  lnot(a) {
    return a.isZero() ? BigInt(0) : BigInt(1)
  }
