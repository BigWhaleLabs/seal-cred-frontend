import { ExternalSCERC721Ledger__factory } from '@big-whale-labs/seal-cred-ledger-contract'
import { Web3Provider } from '@ethersproject/providers'
import BaseBadgeBuilder from 'helpers/BaseBadgeBuilder'
import ERC721Proof from 'helpers/ERC721Proof'
import env from 'helpers/env'

export default class ERC721BadgeBuilder extends BaseBadgeBuilder {
  constructor(provider: Web3Provider) {
    super(provider, env.VITE_EXTERNAL_SC_ERC721_LEDGER_CONTRACT_ADDRESS)
  }

  createLedger(provider: Web3Provider, address: string) {
    return ExternalSCERC721Ledger__factory.connect(
      address,
      provider.getSigner(0)
    )
  }

  create(proof: ERC721Proof, data: string, signature: string) {
    return this.mint(proof.result, data, signature)
  }
}
